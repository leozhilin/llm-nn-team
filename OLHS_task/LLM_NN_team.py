...
def evaluate_allocation(epoch, classifier, allocation_system, test_loader):
    ...
    return system_accuracy, time_cost
...
def my_approach(train_loader, test_loader):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_allocation(allocation_input_list, h_list, allocation_system, allocation_system_optimizer,
                         allocation_system_loss_fn, allocation_system_scheduler)
        system_accuracy, system_time_cost = evaluate_allocation(epoch, classifier, allocation_system, test_loader)
        ...
        if system_accuracy > best_accuracy:
            ...
            best_accuracy_result = [system_accuracy, system_time_cost]
        if system_time_cost < best_time_cost:
            best_time_cost = system_time_cost
            best_time_cost_result = [system_accuracy, system_time_cost]
...
if __name__ == '__main__':
    ...
    OLHS_WITH_PREDS_PATH = 'OLHS_data_ChatGLM3.json' #你需要将此路径替换为数据文件所在的路径
    SEED = 67
    EXPERIMENT_EPOCH = 3
    ...
    print("-------------------------------------------baseline--------------------------------------------")
    print("llm_accuracy:", llm_accuracy, "llm_time_cost:", llm_time_cost)
    print(f"nn accuracy : {classifier_avg_best_accuracy_result}({classifier_stdev_best_accuracy_result}), nn time cost : {classifier_avg_best_time_cost_result}({classifier_stdev_best_time_cost_result})\n")

    print("------------------------------------------my approach------------------------------------------")
    print(f"best accuracy result : \n"
          f"system accuracy : {avg_best_accuracy_result[0]}({stdev_best_accuracy_result[0]}), system time cost : {avg_best_accuracy_result[1]}({stdev_best_accuracy_result[1]})\n")

    print(f"best time cost result : \n"
          f"system accuracy : {avg_best_time_cost_result[0]}({stdev_best_time_cost_result[0]}), system time cost : {avg_best_time_cost_result[1]}({stdev_best_time_cost_result[1]})\n")

    print("-------------------------------------------C_E_Team--------------------------------------------")
    print(f"best accuracy result : \n"
          f"system accuracy : {CET_avg_best_accuracy_result[0]}({CET_stdev_best_accuracy_result[0]}), system time cost : {CET_avg_best_accuracy_result[1]}({CET_stdev_best_accuracy_result[1]})\n")
    print(f"best time cost result : \n"
          f"system accuracy : {CET_avg_best_time_cost_result[0]}({CET_stdev_best_time_cost_result[0]}), system time cost : {CET_avg_best_time_cost_result[1]}({CET_stdev_best_time_cost_result[1]})\n")
